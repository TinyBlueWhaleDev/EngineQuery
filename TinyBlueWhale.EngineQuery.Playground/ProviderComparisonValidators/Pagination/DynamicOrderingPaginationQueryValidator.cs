...
{
    /// <summary>
    /// Validates dynamic primary ordering followed by deterministic secondary ordering
    /// and provider-specific Skip/Take pagination.
    ///
    /// SQL Server:
    /// SELECT [u].[user_id] AS [UserId], [u].[email]
    /// FROM [users] AS [u]
    /// ORDER BY [u].[email] DESC, [u].[user_id] ASC
